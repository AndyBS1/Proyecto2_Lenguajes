...
-- Ocultar PINs Preguntar si así o con asteriscos ----------------------------
...
registerUser :: IO ()
registerUser = do
  putStrLn "Bienvenido al Administrador de Contraseñas."
...
requestPIN = do
  ...
  if fileExists
    then do
      ...
      putStrLn "No se encontró información de usuario, primero debes registrarte."
      return False
    ...
    "1" -> do
      ...
      if accessGranted
        then do
          ...
          -- AGREGAR LO DE LAS CONTRASEÑAASSSSSSSSSSS ----------------
          putStrLn "Pendiente............."
          showMenu
          handleMenuSelection
        else do
